...
The method is described in a preprint [@whiteside2026ar2]. No external funding
was received for this work. The author is solely responsible for the software,
the benchmark and this paper: conceptualisation, methodology, software, formal
analysis, writing and visualisation.

# Data and code availability

The package is on PyPI as `par2-circadian` and its source, tests, benchmark and
this paper are in the repository, archived at Zenodo
(doi:10.5281/zenodo.21963192) and registered as RRID:SCR\_028837. The bundled
example dataset (`data/example_circadian.csv`) is **simulated**, not measured,
and is generated by the script beside it; it exists to exercise the API, not to
support any biological claim. Every number quoted above is reproduced by the
scripts in `benchmarks/`, which write the tables they are read from; the
comparator arms require R with `MetaCycle` and `rain`.

# Competing interests

The author holds a pending UK patent application (GB2518973.9) covering the
AR(2) eigenvalue methodology, and offers commercial licences to related
software. This software is released under Apache-2.0, whose patent grant
(section 3) covers use of this implementation; it is not a licence to practise
the methodology by other means. The author also operates a web platform built
on the same method, which is separately licensed and is not part of this
submission.

# AI usage statement

This work was produced with substantial AI assistance, which the author
considers material enough to state in detail rather than in a single sentence.

*Software and benchmark.* AI coding agents — most recently Devin (Cognition
AI), and Replit Agent for earlier work on the author's platform — wrote most of
the implementation, the test suite and the benchmark harness from
specifications set by the author, who defined what was to be computed, what
would count as a correct answer, and which comparisons mattered. The agents
also executed the simulations and produced the numerical results reported here.
The author reviewed the code and results and is responsible for them.

*Manuscript.* This paper was drafted by an AI agent under the author's
direction, then reviewed and approved by the author, who is its sole author.
Bibliographic records were retrieved
programmatically from Crossref rather than typed, and checked against the
resolved DOIs, after an audit of the author's earlier manuscripts found
fabricated references produced by unverified AI drafting.

*Boundary.* The AR(2) persistence framework, the decision to measure
persistence rather than rhythmicity, the choice of comparators and scenarios,
and the interpretation of the limitations are the author's. No AI system is an
author, and none is credited as one. Readers should weight the software by its
tests and its benchmark, both of which are executable, rather than by its
provenance.
